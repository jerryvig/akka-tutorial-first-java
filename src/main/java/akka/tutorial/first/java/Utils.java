...
public class Utils {
  ...
  public final static int NUMBER_OF_BALLS = 400; 
  ...
  public static void addGround(float width, float height){
      ...
      bodyDef.position= new Vec2(0.0f,-10f);

      world.createBody(bodyDef).createFixture(fixtureDef);
  }
  
  //This method adds a wall.
  public static void addWall(float posX, float posY, float width, float height){
      ...
      Utils.world.createBody(bodyDef).createFixture(fixtureDef);
  }
  
  //Convert a JavaFX pixel y coordinate to a JBox2D y coordinate
  public static float toPosY(float posY) {
      float y = 100.0f - ((posY * 100.0f*1.0f) /HEIGHT) ;
      return y;
  }
